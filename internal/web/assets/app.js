...
async function loadSettings() { const [health, settings] = await Promise.all([api('/api/admin/health'), api('/api/admin/settings')]); $('#top-status').textContent = health.status.toUpperCase(); const healthPill = $('#health-state'); healthPill.textContent = health.status.toUpperCase(); healthPill.classList.toggle('is-bad', health.status !== 'ready'); $('#health-metrics').innerHTML = `<dt>Provider instances</dt><dd>${health.providers}</dd><dt>Available real models</dt><dd>${health.available_models}</dd><dt>Retired real models</dt><dd>${health.retired_models}</dd><dt>Broken virtual routes</dt><dd>${health.broken_virtual_models}</dd><dt>Persistence</dt><dd>SQLITE / READY</dd>`; $('[name="default_logging_enabled"]', $('#settings-form')).checked = settings.default_logging_enabled; $('[name="default_retention_days"]', $('#settings-form')).value = settings.default_retention_days; await loadGlobalActivity(); }
