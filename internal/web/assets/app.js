...
function renderVirtual() {
  ...
  const html = groupNames.sort((a, b) => a.localeCompare(b)).map(name => {
    ...
    return groupBanner('virtual', name, name, note, `${models.length} model${models.length === 1 ? '' : 's'}`, actions) + groupRows(models.map(model => `<td><code class="model-id">${h(model.canonical_model_id)}</code><span class="meta-line">Stable client identity</span></td><td></td><td><code class="model-id">${h(model.target_provider_name)}/${h(model.target_upstream_model_id)}</code><span class="meta-line">${h(model.target_provider_name)}</span></td><td>${badge(model.available, model.available ? 'Routable' : 'Broken target', model.available ? 'good' : 'bad')}${model.warning ? `<span class="error-text">${h(model.warning)}</span>` : ''}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['1h'])}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['24h'])}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['7d'])}</td><td><div class="actions"><button class="btn btn-small btn-secondary" data-virtual-edit="${h(model.id)}">Remap / edit</button><button class="btn btn-small btn-danger" data-virtual-delete="${h(model.id)}">Delete</button></div></td>`), name, collapsed);
  }).join('');
...
const activityState = { client: null, rows: [], offset: 0, limit: 50, search: '' };
