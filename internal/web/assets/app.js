...
function requestIdentity(row) { if (row.exposed_model && row.exposed_model !== row.requested_model) { return `<code class="model-id">${h(row.requested_model)}</code><span class="meta-line">map &gt; ${h(row.exposed_model)}</span>`; } return `<code class="model-id">${h(row.requested_model)}</code>`; }
