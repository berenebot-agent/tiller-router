...
async function navigate(view) {
  ...
  try { if (view === 'providers') await loadProviders(); if (view === 'models') await loadModels(); if (view === 'virtual') await loadVirtual(); if (view === 'clients') await loadClients(); if (view === 'settings') { await loadSettings(); $('[name="log_error_bodies"]', $('#body-logging-form')).checked = (await api('/api/admin/settings')).log_error_bodies; } }
...
function resolvedActivity(row) { const fixedAttempt = row.resolved_provider ? { provider: row.resolved_provider, model: row.resolved_model || '', latency_ms: row.latency_ms, http_status: row.http_status, error_message: row.error_message, error_body: row.error_body, error_body_truncated: row.error_body_truncated, result: row.http_status >= 200 && row.http_status < 300 ? 'success' : 'failed' } : null; const resolved = row.fallback_used ? '' : fixedAttempt ? `<div class="attempt-sequence">${activityAttempt(fixedAttempt, 0)}</div>` : ''; const sequence = (row.fallback_used || !fixedAttempt) ? attemptSequence(row) : ''; const error = sequence ? '' : (row.error_text ? `<span class="error-text">${h(row.error_text)}</span>` : ''); const requestBody = activityBodyButton('Client request body', row.request_body, row.request_body_truncated); return `${resolved}${sequence}${error}${requestBody}`; }
...
async function saveBodyLoggingSetting() { const checked = $('[name="log_error_bodies"]', $('#body-logging-form')).checked; try { await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ log_error_bodies: checked }) }); flash(checked ? 'Detailed error logging enabled.' : 'Detailed error logging disabled.'); } catch (error) { $('#settings-error').textContent = errorMessage(error); } }
$('[name="log_error_bodies"]', $('#body-logging-form')).addEventListener('change', saveBodyLoggingSetting);
