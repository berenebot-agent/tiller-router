...
function renderProviders() {
  const body = $('#providers-body'); $('#providers-empty').hidden = state.providers.length > 0; body.innerHTML = state.providers.map(provider => `<tr>
    ...
    <td><div class="actions">${['codex-subscription','claude-subscription','github-copilot'].includes(provider.type) && (provider.auth_state === 'connected' || provider.auth_state === 'reconnect_required') ? `<button class="btn btn-small btn-danger" data-provider-disconnect="${h(provider.id)}">Disconnect</button>` : ''}<button class="btn btn-small btn-secondary" data-provider-refresh="${h(provider.id)}">Refresh</button><button class="btn btn-small btn-secondary" data-provider-edit="${h(provider.id)}">Edit</button><button class="btn btn-small btn-danger" data-provider-delete="${h(provider.id)}">Delete</button></div></td></tr>`).join('');
  ...
  $$('[data-provider-disconnect]').forEach(button => button.onclick = () => disconnectProviderOAuth(button.dataset.providerDisconnect));
}
...
function providerFields(provider) {
  ...
  return `<label>Provider name <input name="name" value="${h(provider?.name || '')}" placeholder="openai-main" pattern="[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?"><small>Lowercase namespace used in client-facing model IDs. Leave blank to use provider type (e.g. DeepSeek → deepseek).</small></label>
    ...
    ${isOAuthEdit ? '<div data-provider-reconnect><label>OAuth connection <button type="button" class="btn btn-secondary" data-provider-reconnect-btn>Reconnect</button><small>Start a fresh OAuth sign-in. Replaces any saved token for this provider.</small></label></div>' : ''}
