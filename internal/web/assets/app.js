...
$('#fallback-form').addEventListener('submit', async event => { event.preventDefault(); const button = $('button[type="submit"]', event.currentTarget); button.disabled = true; $('#fallback-error').textContent = ''; try { const values = new FormData(event.currentTarget); await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ fallback_timeout_seconds: Number(values.get('fallback_timeout_seconds')) }) }); flash('Fallback timeout saved.'); } catch (error) { $('#fallback-error').textContent = errorMessage(error); } finally { button.disabled = false; } });
