...
function showLogin() { $('#app').hidden = true; $('#login-shell').hidden = false; state.csrf = ''; }
...
async function navigate(view) {
  state.view = view; $$('.view').forEach(panel => panel.classList.toggle('active', panel.id === `view-${view}`)); $$('[data-view]').forEach(button => button.classList.toggle('active', button.dataset.view === view)); $('#nav-links').classList.remove('open'); $('#mobile-menu').setAttribute('aria-expanded', 'false');
...
function virtualModelFields(model) {
  ...
  return `<div class="row"><label>Virtual group <select name="group_id" ${model ? 'disabled' : ''} required>${groupOptions}</select></label><label>Virtual model name <input name="name" value="${h(model?.name || '')}" placeholder="coding" required></label></div><label>Target provider <select name="target_provider_id" required>${providerOptions}</select></label><label>Target real model <select name="target_model_id" required></select><small>Only models belonging to the selected provider are eligible.</small></label>${model ? '<label class="confirm-check"><input name="confirm" type="checkbox"> <span>Confirm if changing the virtual model name; this is a breaking client-facing rename.</span></label>' : ''}`;
}
function openVirtualModel(model = null) { if (!state.groups.length) { flash('Create a virtual provider group first.', 'info'); openVirtualGroup(); return; } if (!state.models.length) { flash('Discover at least one real model before creating a virtual route.', 'info'); return; } openEntity({ eyebrow: model ? 'IMMEDIATE REMAP' : 'NEW STABLE IDENTITY', title: model ? `Edit ${model.canonical_model_id}` : 'Create virtual model', fields: virtualModelFields(model), submit: model ? 'Apply immediately' : 'Create route', onMount: form => { const provider = $('[name="target_provider_id"]', form), target = $('[name="target_model_id"]', form); const options = () => { const matches = state.models.filter(item => item.provider_id === provider.value); target.innerHTML = matches.map(item => `<option value="${h(item.id)}" ${model?.target_model_id === item.id ? 'selected' : ''}>${h(item.upstream_model_id)}${item.available ? '' : ' — retired'}</option>`).join(''); }; provider.addEventListener('change', options); options(); }, onSubmit: async form => { const values = new FormData(form); const payload = { name: values.get('name'), target_provider_id: values.get('target_provider_id'), target_model_id: values.get('target_model_id') }; if (model) { payload.confirm_breaking_change = values.get('confirm') === 'on'; await api(`/api/admin/virtual-models/${model.id}`, { method: 'PATCH', body: JSON.stringify(payload) }); flash('Virtual route remapped. New requests use the target immediately.'); } else { payload.group_id = values.get('group_id'); await api('/api/admin/virtual-models', { method: 'POST', body: JSON.stringify(payload) }); flash('Virtual route created.'); } await loadVirtual(); } }); }
...
async function handleEntitySubmit(event) { event.preventDefault(); const form = event.currentTarget, button = $('#dialog-submit'); if (!form.reportValidity()) return; button.disabled = true; $('#dialog-error').textContent = ''; try { await entitySubmit(form); $('#form-dialog').close(); } catch (error) { $('#dialog-error').textContent = errorMessage(error); } finally { button.disabled = false; } }
...
(async function initialise() { try { const session = await api('/api/admin/session'); showApp(session); } catch { showLogin(); } })();
