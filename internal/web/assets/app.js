...
function renderVirtual() {
  ...
  const html = groupNames.sort((a, b) => a.localeCompare(b)).map(name => {
    ...
    return groupBanner('virtual', name, name, note, `${models.length} model${models.length === 1 ? '' : 's'}`, actions) + groupRows(models.map(model => { const targets = model.targets || []; const summary = targets.length ? `<div class="target-summary">${targets.map((target, index) => `<span class="meta-line">${index + 1}. ${resolutionIndicator(target)}${h(target.provider_name)}/${h(target.upstream_model_id)}${target.enabled ? '' : ' (disabled)'}</span>`).join('')}</div>` : `<span class="meta-line">${resolutionIndicator({provider_name:model.target_provider_name,upstream_model_id:model.target_upstream_model_id})}</span><code class="model-id">${h(model.target_provider_name || '')}/${h(model.target_upstream_model_id || '')}</code>`; return `<td><code class="model-id">${h(model.canonical_model_id)}</code><span class="meta-line">${h(model.routing_mode === 'ordered_fallback' ? 'Ordered fallback' : 'Fixed')}</span></td><td></td><td>${summary}</td><td>${badge(model.available, model.available ? 'Routable' : 'Broken target', model.available ? 'good' : 'bad')}${model.warning ? `<span class="error-text">${h(model.warning)}</span>` : ''}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['1h'])}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['24h'])}</td><td>${mtok(state.usage?.virtual_models?.[model.canonical_model_id]?.['7d'])}</td><td><div class="actions"><button class="btn btn-small btn-secondary" data-model-activity="${h(model.canonical_model_id)}">Activity</button><button class="btn btn-small btn-secondary" data-virtual-capabilities="${h(model.id)}">Capabilities</button><button class="btn btn-small btn-secondary" data-virtual-edit="${h(model.id)}">Remap / edit</button><button class="btn btn-small btn-danger" data-virtual-delete="${h(model.id)}">Delete</button></div></td>`; }), name, collapsed);
  }).join('');
...
function openCapabilities(model) {
  ...
  $('#capabilities-content').innerHTML = `<div class="notice compact"><span>CATALOGUE RULE</span><p>Clients receive the minimum context and output limits across enabled, available targets. An ordered-fallback route can therefore advertise less than its first target.</p></div><div class="capability-effective"><p class="eyebrow">ADVERTISED TO HERMES / V1 MODELS</p><div class="capability-grid"><div><small>Context window</small><strong>${capabilityNumber(effectiveContext)}</strong></div><div><small>Max output</small><strong>${capabilityNumber(effectiveOutput)}</strong></div></div></div><div class="capability-targets"><p class="eyebrow">CONFIGURED TARGETS</p>${targets.length ? targets.map((target, index) => `<div class="capability-target"><div><strong>${String(index + 1).padStart(2, '0')} · ${h(target.provider_name)}/${h(target.upstream_model_id)}</strong><span class="meta-line">${target.native_protocol ? h(target.native_protocol) : 'Provider default'} · ${target.enabled && target.available ? 'eligible' : h(target.warning || 'not eligible')}</span></div><div class="capability-values"><span><small>Context</small><b>${capabilityNumber(target.context_length)}</b></span><span><small>Output</small><b>${capabilityNumber(target.max_output_tokens)}</b></span></div></div>`).join('') : '<p class="meta-line">No targets configured.</p>'}</div><p class="form-error" id="capabilities-refresh-error" role="alert"></p>`;
...
function combobox({ input, hidden, options, placeholder, onSelect }) {
  ...
  const render = (showAll = false) => {
    ...
    list.innerHTML = items.map((opt, i) => `<li role="option" data-i="${i}" ${i === active ? 'aria-selected="true"' : ''}>${h(opt.label)}${opt.muted ? '<small> — retired</small>' : ''}</li>`).join(''); const box=input.getBoundingClientRect(); list.style.left=`${box.left}px`; list.style.top=`${box.bottom+3}px`; list.style.width=`${box.width}px`;
...
function openVirtualModel(model = null) { if (!state.models.length) { flash('Discover at least one real model before creating a virtual route.', 'info'); return; } openEntity({ eyebrow: model ? 'ROUTING POLICY' : 'NEW STABLE IDENTITY', title: model ? `Edit ${model.canonical_model_id}` : 'Create virtual model', fields: virtualModelFields(model), submit: model ? 'Apply immediately' : 'Create route', onMount: form => {
  const fixed = $('[data-fixed-target]', form), fallback = $('[data-fallback-targets]', form), mode = $('[name="routing_mode"]', form), addButton = $('[data-target-add]', form);
  ...
  fixed.append(makePicker(targets[0])); targets.forEach(addFallback); const syncMode=()=>{ const ordered=mode.value==='ordered_fallback'; fixed.hidden=ordered; fallback.hidden=!ordered; addButton.hidden=!ordered; }; mode.onchange=syncMode; syncMode(); addButton.onclick=()=>{ if($$('.target-row',fallback).length<5) addFallback(); else flash('The admin UI supports up to five targets.', 'info'); };
...
}, onSubmit: async form => { const values = new FormData(form); const ordered=values.get('routing_mode')==='ordered_fallback'; const rows=ordered ? $$('.target-row',form) : [ $('[data-fixed-target]',form) ]; const targets=rows.map(row=>({provider_model_id:$('[name="target_model"]',row).value,enabled:true})); if(targets.some(target=>!target.provider_model_id)) throw new Error('Choose a target model.'); const payload = { name: values.get('name'), routing_mode: values.get('routing_mode'), targets }; if(!ordered) payload.fixed_target_id=targets[0].provider_model_id; if (model) { payload.confirm_breaking_change = values.get('confirm') === 'on'; await api(`/api/admin/virtual-models/${model.id}`, { method: 'PATCH', body: JSON.stringify(payload) }); await refreshProviderCatalogues(targets.map(target => state.models.find(item => item.id === target.provider_model_id)?.provider_id)); flash('Virtual routing updated and target capabilities refreshed. New requests use it immediately.'); } else { const groupID = values.get('group_id'); if (groupID) payload.group_id = groupID; else payload.group_name = values.get('group_name'); await api('/api/admin/virtual-models', { method: 'POST', body: JSON.stringify(payload) }); flash('Virtual route created.'); } await loadVirtual(); } }); }
...
function renderGlobalActivity() { $('#global-activity-empty').hidden = globalActivityState.rows.length > 0; $('#global-activity-body').innerHTML = globalActivityState.rows.map(row => `<tr><td><span class="meta-line">${date(row.created_at)}</span></td><td><strong>${h(row.client_name)}</strong></td><td><code class="model-id">${h(row.requested_model)}</code></td><td>${row.resolved_provider ? `<code class="model-id">${h(row.resolved_provider)}/${h(row.resolved_model || '')}</code>` : '<span class="meta-line">—</span>'}${badge(row.http_status >= 200 && row.http_status < 300, row.http_status, row.http_status >= 200 && row.http_status < 300 ? 'good' : 'bad')}${row.error_text ? `<span class="error-text">${h(row.error_text)}</span>` : ''}</td><td><span class="protocol">${h(row.protocol)}</span>${row.streaming ? '<span class="protocol">stream</span>' : ''}</td><td><span class="meta-line">${row.latency_ms} ms</span></td><td><span class="meta-line">${row.input_tokens ?? '—'} / ${row.output_tokens ?? '—'}</span></td><td><code class="model-id">${h(row.client_request_id)}</code></td></tr>`).join(''); $('#global-activity-count').textContent = globalActivityState.rows.length ? `${globalActivityState.offset + 1}–${globalActivityState.offset + globalActivityState.rows.length}` : '0 results'; $('#global-activity-prev').disabled = globalActivityState.offset === 0; $('#global-activity-next').disabled = !globalActivityState.hasMore; }
