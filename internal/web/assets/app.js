...
async function openModelActivity(model, kind) { activityState.kind = kind; activityState.client = null; activityState.modelID = model.id; activityState.modelName = model.canonical_model_id; activityState.offset = 0; activityState.search = model.canonical_model_id; $('#activity-search').value = model.canonical_model_id; $('#activity-title').textContent = `${model.canonical_model_id} activity`; await loadActivity(); $('#activity-dialog').showModal(); }
...
async function loadSettings() { const [health, settings] = await Promise.all([api('/api/admin/health'), api('/api/admin/settings')]); $('#top-status').textContent = health.status.toUpperCase(); $('[name="default_logging_enabled"]', $('#settings-form')).checked = settings.default_logging_enabled; $('[name="default_retention_days"]', $('#settings-form')).value = settings.default_retention_days; $('[name="fallback_timeout_seconds"]', $('#fallback-form')).value = settings.fallback_timeout_seconds; await loadGlobalActivity(); }
async function saveSettings() { const settingsForm = $('#settings-form'), fallbackForm = $('#fallback-form'); if (!settingsForm.reportValidity() || !fallbackForm.reportValidity()) return; const settingsValues = new FormData(settingsForm), fallbackValues = new FormData(fallbackForm); const buttons = [$('#save-settings-top'), $('#save-settings-bottom')]; buttons.forEach(b => b.disabled = true); $('#settings-error').textContent = ''; $('#fallback-error').textContent = ''; try { await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ default_logging_enabled: settingsValues.get('default_logging_enabled') === 'on', default_retention_days: Number(settingsValues.get('default_retention_days')), fallback_timeout_seconds: Number(fallbackValues.get('fallback_timeout_seconds')) }) }); flash('Settings saved.'); } catch (error) { const message = errorMessage(error); $('#settings-error').textContent = message; $('#fallback-error').textContent = message; } finally { buttons.forEach(b => b.disabled = false); } }
