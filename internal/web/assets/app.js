...
async function loadModels(search = $('#model-search').value) { const [result, usage, providersResult] = await Promise.all([api(`/api/admin/models?all=1&search=${encodeURIComponent(search || '')}`), api('/api/admin/usage'), api('/api/admin/providers?limit=200')]); state.models = result.data; state.usage = usage; state.providers = providersResult.data; renderModels(); }
...
function activityBodyButton(label, body, truncated) { if (!body) return ''; const key = ++errorDetailSeq; errorDetails.set(key, { title: label, body: `${body}${truncated ? '\n\n[truncated]' : ''}` }); return `<button type="button" class="activity-attempt-message is-clickable" data-error-key="${key}" title="Show ${h(label.toLowerCase())}">${h(label)}${truncated ? ' (truncated)' : ''}</button>`; }
const latencyFailureClasses = new Set(['upstream_timeout', 'client_timeout', 'upstream_response_too_large', 'upstream_unreachable', 'upstream_read_error']);
function activityAttempt(attempt, index) { const route = `${attempt.provider}/${attempt.model}`; const status = attempt.http_status ? `${attempt.result === 'failed' ? 'Failed · ' : ''}HTTP ${attempt.http_status}` : h(attempt.result); let message = ''; if (attempt.result === 'failed') { if (latencyFailureClasses.has(attempt.failure_class)) { message = `<small class="activity-attempt-message">${h(attempt.error_message || attempt.failure_class)} · ${attempt.latency_ms} ms</small>`; } else { const key = ++errorDetailSeq; const parts = []; if (attempt.error_message) parts.push(attempt.error_message); if (attempt.failure_class) parts.push(`Resolver: ${attempt.failure_class}`); if (attempt.error_body) parts.push(`Provider error body:\n${attempt.error_body}${attempt.error_body_truncated ? '\n\n[truncated]' : ''}`); errorDetails.set(key, { title: `${route} · ${attempt.http_status ? `HTTP ${attempt.http_status}` : attempt.result}`, body: parts.join('\n\n') || '(no error details)' }); message = `<button type="button" class="activity-attempt-message is-clickable" data-error-key="${key}" title="Show full error">${h(attempt.error_message || attempt.failure_class)}</button>`; } } else if (attempt.error_message) { message = `<small class="activity-attempt-message">${h(attempt.error_message)}</small>`; } return `<div class="activity-attempt ${attempt.result === 'success' ? 'attempt-success' : attempt.result === 'failed' ? 'attempt-failed' : 'attempt-neutral'}"><span class="attempt-number">${String(index + 1).padStart(2, '0')}</span><span class="attempt-route"><code title="${h(route)}">${h(route)}</code>${message}</span><span class="attempt-status">${status}</span></div>`; }
...
function resolvedActivity(row) { const fixedAttempt = row.resolved_provider ? { provider: row.resolved_provider, model: row.resolved_model || '', failure_class: row.error_text, latency_ms: row.latency_ms, http_status: row.http_status, error_message: row.error_message, error_body: row.error_body, error_body_truncated: row.error_body_truncated, result: row.http_status >= 200 && row.http_status < 300 ? 'success' : 'failed' } : null; const resolved = row.fallback_used ? '' : fixedAttempt ? `<div class="attempt-sequence">${activityAttempt(fixedAttempt, 0)}</div>` : ''; const sequence = (row.fallback_used || !fixedAttempt) ? attemptSequence(row) : ''; const error = sequence ? '' : (row.error_text ? `<span class="error-text">${h(row.error_text)}</span>` : ''); const requestBody = activityBodyButton('Client request body', row.request_body, row.request_body_truncated); return `${resolved}${sequence}${error}${requestBody}`; }
