...
function openClient(client = null) {
  ...
  const typeField = `<label>Type <select name="type"><option value="catalogue" ${client?.type !== 'single' ? 'selected' : ''}>Catalogue</option><option value="single" ${client?.type === 'single' ? 'selected' : ''}>Single</option></select><small data-type-tip>${client?.type === 'single' ? 'Expose one model to the client and route all requests to that single model.' : 'Choose which real and virtual models the client can access.'}</small></label>`;
  ...
  openEntity({
    ...
    onMount: form => {
      const typeSelect = $('[name="type"]', form), fields = $('[data-single-fields]', form), name = $('[name="single_model_name"]', form), tip = $('[data-type-tip]', form);
      mountSingleTargetPicker($('[data-single-target]', form), client);
      const syncType = () => { fields.hidden = typeSelect.value !== 'single'; name.required = !fields.hidden; tip.textContent = typeSelect.value === 'single' ? 'Expose one model to the client and route all requests to that single model.' : 'Choose which real and virtual models the client can access.'; };
