...
$('#copy-secret').onclick = async () => { const text = $('#secret-value').textContent; const state = $('#copy-state'); if (window.isSecureContext && navigator.clipboard?.writeText) { try { await navigator.clipboard.writeText(text); state.textContent = 'Copied to clipboard.'; return; } catch { /* fall through to legacy path */ } } const ta = document.createElement('textarea'); ta.value = text; ta.setAttribute('readonly', ''); ta.style.position = 'fixed'; ta.style.top = '0'; ta.style.left = '0'; ta.style.opacity = '0'; document.body.appendChild(ta); ta.focus(); ta.select(); let ok = false; try { ok = document.execCommand('copy'); } catch { /* ok stays false */ } document.body.removeChild(ta); state.textContent = ok ? 'Copied to clipboard.' : 'Clipboard access was denied. Select and copy the key manually.'; };
