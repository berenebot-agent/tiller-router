...
function activityAttempt(attempt, index) { const route = `${attempt.provider}/${attempt.model}`; const status = attempt.http_status ? `HTTP ${attempt.http_status}` : h(attempt.result); const cls = attempt.result === 'success' ? 'attempt-success' : attempt.result === 'failed' ? 'attempt-failed' : 'attempt-neutral'; let clickAttr = ''; if (attempt.result === 'failed') { const key = ++errorDetailSeq; const parts = []; if (attempt.error_message) parts.push(attempt.error_message); if (attempt.failure_class) parts.push(`Resolver: ${attempt.failure_class}`); if (attempt.latency_ms) parts.push(`Latency: ${attempt.latency_ms} ms`); errorDetails.set(key, { title: `${route} · ${status}`, body: parts.join('\n\n') || '(no error details)' }); clickAttr = ` data-error-key="${key}"`; } return `<div class="activity-attempt ${cls}"${clickAttr}><span class="attempt-number">${String(index + 1).padStart(2, '0')}</span><span class="attempt-route"><code title="${h(route)}">${h(route)}</code></span><span class="attempt-status">${status}</span></div>`; }
