...
function providerFields(provider) {
  ...
  return `<label>Provider name <input name="name" value="${h(provider?.name || '')}" placeholder="openai-main (defaults to provider type)" pattern="[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?"><small>Lowercase namespace used in client-facing model IDs. Leave blank to use provider type (e.g. DeepSeek → deepseek).</small></label>
...
function openProvider(provider = null) {
  openEntity({ eyebrow: provider ? 'EDIT UPSTREAM' : 'REGISTER UPSTREAM', title: provider ? `Edit ${provider.name}` : 'Add provider', fields: providerFields(provider), submit: provider ? 'Save provider' : 'Add & discover', onMount: form => { const select = $('[name="type"]', form); const protocolConfig = $('[data-protocol-config]', form); const showProtocols = () => protocolConfig.hidden = !['generic-openai','vllm'].includes(provider?.type || select.value); if (!provider) { const base = $('[name="base_url"]', form); const nameInput = $('[name="name"]', form); const credential = $('[name="credential"]', form); const createWrap = $('[data-credential-create]', form); const apply = () => { const item = state.providerTypes.find(t => t.type === select.value); if (!base.value || base.dataset.auto === 'true') { base.value = item?.default_base_url || ''; base.dataset.auto = 'true'; } if (!nameInput.value || nameInput.dataset.auto === 'true') { nameInput.value = select.value; nameInput.dataset.auto = 'true'; } const needsCred = Boolean(item?.credential_needed); credential.required = needsCred; if (createWrap) { createWrap.hidden = !needsCred; if (createWrap.hidden) credential.value = ''; } showProtocols(); }; base.addEventListener('input', () => base.dataset.auto = 'false'); nameInput.addEventListener('input', () => nameInput.dataset.auto = 'false'); select.addEventListener('change', apply); apply(); } else { const replaceWrap = $('[data-credential-replace]', form); if (replaceWrap) { const desc = state.providerTypes.find(t => t.type === provider.type); replaceWrap.hidden = !desc?.credential_needed; } showProtocols(); const nameInput = $('[name="name"]', form), confirmWrap = $('[data-confirm-wrap]', form); const syncConfirm = () => { confirmWrap.hidden = nameInput.value === provider.name; if (confirmWrap.hidden) { const cb = $('[name="confirm_breaking_change"]', form); if (cb) cb.checked = false; } }; nameInput.addEventListener('input', syncConfirm); syncConfirm(); } }, onSubmit: async form => {
...
function resolutionIndicator(target) {
  const last = state.usage?.target_last_outcome?.[`${target.provider_name}/${target.upstream_model_id}`];
  const status = !last || !last.at
