...
function renderProviders() {
  const body = $('#providers-body'); $('#providers-empty').hidden = state.providers.length > 0; body.innerHTML = state.providers.map(provider => `<tr>
    ...
    <td><div class="actions">${['codex-subscription','claude-subscription','github-copilot'].includes(provider.type) ? `<button class="btn btn-small btn-secondary" data-provider-oauth="${h(provider.id)}">${provider.credential_configured ? 'Reconnect' : 'Connect'}</button>` : ''}<button class="btn btn-small btn-secondary" data-provider-refresh="${h(provider.id)}">Refresh</button><button class="btn btn-small btn-secondary" data-provider-edit="${h(provider.id)}">Edit</button><button class="btn btn-small btn-danger" data-provider-delete="${h(provider.id)}">Delete</button></div></td></tr>`).join('');
  ...
  $$('[data-provider-oauth]').forEach(button => button.onclick = () => connectProviderOAuth(button.dataset.providerOauth));
}
...
async function refreshProvider(id) { const button = $(`[data-provider-refresh="${CSS.escape(id)}"]`); button.disabled = true; try { await api(`/api/admin/providers/${id}/refresh`, { method: 'POST' }); flash('Catalogue refresh completed.'); await loadProviders(); } catch (error) { flash(errorMessage(error), 'error'); await loadProviders(); } finally { button.disabled = false; } }
 async function connectProviderOAuth(id) { try { const result = await api(`/api/admin/providers/${id}/oauth/start`, { method: 'POST' }); if (result.flow === 'device_code') { showGitHubDeviceDialog(id, result); return; } window.open(result.authorization_url, 'tiller-oauth-auth', 'popup,width=520,height=720,resizable=yes,scrollbars=yes'); showOAuthCallbackDialog(id, result.authorization_url); } catch (error) { flash(errorMessage(error), 'error'); } }
 ...
 function showOAuthCallbackDialog(id, authorizationURL) { openEntity({ eyebrow: 'CODEX SUBSCRIPTION', title: 'Finish OpenAI sign-in', submit: 'Connect Codex', fields: `<p>1. Finish signing in in the small OpenAI window.<br>2. When it redirects to <code>http://localhost:1455</code>, copy the complete URL from your browser address bar.<br>3. Paste that URL below. The page may not load; that is expected.</p><label>Authorization URL <textarea readonly rows="4">${h(authorizationURL)}</textarea></label><label>Redirected URL <textarea name="redirected_url" rows="3" required placeholder="http://localhost:1455/auth/callback?code=...&state=..."></textarea></label>`, onSubmit: async form => { const value = new FormData(form).get('redirected_url'); await api(`/api/admin/providers/${id}/oauth/callback`, { method: 'POST', body: JSON.stringify({ redirected_url: value }) }); flash('Codex connected.'); await loadProviders(); }}); }
...
function activityBodyButton(label, body, truncated) { if (!body) return ''; const key = ++errorDetailSeq; errorDetails.set(key, { title: label, sections: [{ heading: label, body: `${body}${truncated ? '\n\n[truncated]' : ''}` }] }); return `<button type="button" class="activity-attempt-message is-clickable" data-error-key="${key}" title="Show ${h(label.toLowerCase())}">${h(label)}${truncated ? ' (truncated)' : ''}</button>`; }
function activityAttempt(attempt, index, row) { const route = `${attempt.provider}/${attempt.model}`; const status = attempt.http_status ? `${attempt.result === 'failed' ? 'Failed · ' : ''}HTTP ${attempt.http_status}` : h(attempt.result); let link = ''; if (attempt.result === 'failed') { const requestBody = row?.request_body ?? null; const errorParts = [`Failure class: ${attempt.failure_class || '—'}`, `Latency: ${attempt.latency_ms} ms`, `Attempt: ${index + 1}`]; if (!attempt.error_message && !attempt.error_body) errorParts.push('No error detail was recorded for this attempt.'); if (attempt.error_message) errorParts.push('', attempt.error_message); if (attempt.error_body) errorParts.push('', `${attempt.error_body}${attempt.error_body_truncated ? '\n\n[truncated]' : ''}`); const sections = [{ heading: 'Client request', body: requestBody ? `${requestBody}${row.request_body_truncated ? '\n\n[truncated]' : ''}` : 'Request body logging is off (Settings → Logging Settings → Retain failed request and error bodies).' }, { heading: 'Provider error', body: errorParts.join('\n') }]; const key = ++errorDetailSeq; errorDetails.set(key, { title: `${route} · ${attempt.http_status ? `HTTP ${attempt.http_status}` : attempt.result}`, sections }); const truncated = attempt.error_body_truncated || row?.request_body_truncated; link = `<button type="button" class="activity-attempt-message is-clickable" data-error-key="${key}" title="Show request and error details">Details${truncated ? ' (truncated)' : ''}</button>`; } return `<div class="activity-attempt ${attempt.result === 'success' ? 'attempt-success' : 'attempt-failed'}"><span class="attempt-number">${String(index + 1).padStart(2, '0')}</span><span class="attempt-route"><code title="${h(route)}">${h(route)}</code>${link}</span><span class="attempt-latency">${attempt.latency_ms} ms</span><span class="attempt-status">${status}</span></div>`; }
// Attempt/request detail store. Each failed attempt box renders a single
// "Details" link; the request body (row-level, one per logical request) and
// the provider error (per-attempt message + body, or recorded metadata when
// nothing was stored) live here keyed by data-error-key and render as two
// boxes in #error-dialog. The store is rebuilt on every activity render so
// keys never go stale.
let errorDetailSeq = 0;
let currentErrorKey = 0;
const errorDetails = new Map();
function openErrorDetail(key) { const detail = errorDetails.get(Number(key)); if (!detail) return; currentErrorKey = Number(key); $('#error-title').textContent = detail.title; const content = $('#error-content'); content.innerHTML = ''; detail.sections.forEach(s => { const sec = document.createElement('section'); sec.className = 'error-section'; const heading = document.createElement('h3'); heading.textContent = s.heading; const pre = document.createElement('pre'); const code = document.createElement('code'); code.textContent = s.body; pre.appendChild(code); sec.appendChild(heading); sec.appendChild(pre); content.appendChild(sec); }); $('#error-copy-state').textContent = ''; $('#copy-error').hidden = !(window.isSecureContext && navigator.clipboard?.writeText); $('#error-dialog').showModal(); }
...
$('#copy-error').onclick = async () => { const detail = errorDetails.get(currentErrorKey); const state = $('#error-copy-state'); if (!detail || !(window.isSecureContext && navigator.clipboard?.writeText)) return; const text = detail.sections.map(s => `${s.heading}\n${s.body}`).join('\n\n'); try { await navigator.clipboard.writeText(text); state.textContent = 'Copied to clipboard.'; } catch { state.textContent = 'Clipboard copy was denied — select the text and press Ctrl/Cmd+C.'; } };
$('#close-error').onclick = $('#done-error').onclick = () => $('#error-dialog').close();
function attemptSequence(row) { if (!row.attempts?.length) return ''; return `<div class="attempt-sequence">${row.attempts.map((attempt, i) => activityAttempt(attempt, i, row)).join('')}</div>`; }
function resolvedActivity(row) { const fixedAttempt = row.resolved_provider ? { provider: row.resolved_provider, model: row.resolved_model || '', latency_ms: row.latency_ms, http_status: row.http_status, error_message: row.error_message, error_body: row.error_body, error_body_truncated: row.error_body_truncated, result: row.http_status >= 200 && row.http_status < 300 ? 'success' : 'failed' } : null; const resolved = row.fallback_used ? '' : fixedAttempt ? `<div class="attempt-sequence">${activityAttempt(fixedAttempt, 0, row)}</div>` : ''; const sequence = (row.fallback_used || !fixedAttempt) ? attemptSequence(row) : ''; const error = sequence ? '' : (row.error_text ? `<span class="error-text">${h(row.error_text)}</span>` : ''); const hasFailed = (row.attempts || []).some(a => a.result === 'failed') || (!row.fallback_used && fixedAttempt && fixedAttempt.result === 'failed'); const requestBody = hasFailed ? '' : activityBodyButton('Client request body', row.request_body, row.request_body_truncated); return `${resolved}${sequence}${error}${requestBody}`; }
...
async function saveBodyLoggingSetting() { const checked = $('[name="log_error_bodies"]', $('#settings-form')).checked; try { await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ log_error_bodies: checked }) }); flash(checked ? 'Detailed error logging enabled.' : 'Detailed error logging disabled.'); } catch (error) { $('#settings-error').textContent = errorMessage(error); } }
$('[name="log_error_bodies"]', $('#settings-form')).addEventListener('change', saveBodyLoggingSetting);
