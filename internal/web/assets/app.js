...
function groupBanner(kind, key, label, note, count) { const collapsed = (kind === 'models' ? collapsedModels : collapsedVirtual).has(key); return `<tr class="group-toggle" data-group-toggle="${kind}" data-group-key="${h(key)}" data-expanded="${collapsed ? 'false' : 'true'}" aria-expanded="${collapsed ? 'false' : 'true'}"><td colspan="5"><span class="group-arrow">${collapsed ? GROUP_ARROW.down : GROUP_ARROW.up}</span><span class="group-label">${h(label)}</span><span class="count-badge">${h(count)}</span><span class="meta-line">${h(note)}</span></td></tr>`; }
...
function renderVirtual() {
  ...
  const html = [...byGroup.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([group, models]) => { const collapsed = collapsedVirtual.has(group); const broken = models.filter(m => !m.available).length; const note = broken ? `${broken} broken target` : 'group'; return groupBanner('virtual', group, group, note, `${models.length} model${models.length === 1 ? '' : 's'}`) + groupRows(models.map(model => `<td><code class="model-id">${h(model.canonical_model_id)}</code><span class="meta-line">Stable client identity</span></td><td></td><td><code class="model-id">${h(model.target_provider_name)}/${h(model.target_upstream_model_id)}</code><span class="meta-line">${h(model.target_provider_name)}</span></td><td>${badge(model.available, model.available ? 'Routable' : 'Broken target', model.available ? 'good' : 'bad')}${model.warning ? `<span class="error-text">${h(model.warning)}</span>` : ''}</td><td><div class="actions"><button class="btn btn-small btn-secondary" data-virtual-edit="${h(model.id)}">Remap / edit</button><button class="btn btn-small btn-danger" data-virtual-delete="${h(model.id)}">Delete</button></div></td>`), group, collapsed); }).join('');
