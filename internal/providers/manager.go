...
func (m *Manager) applyCatalogue(ctx context.Context, providerID string, models []Model) error {
	...
	for _, model := range models {
		...
		if err == sql.ErrNoRows {
			...
			if _, err = tx.ExecContext(ctx, `INSERT INTO provider_models(id,provider_id,upstream_model_id,display_name,context_length,max_output_tokens,available,first_seen_at,last_seen_at,created_at,updated_at) VALUES(?,?,?,?,?,?,1,?,?,?,?)`, modelID, providerID, model.ID, model.DisplayName, nullableInt(model.ContextLength), nullableInt(model.MaxOutputTokens), now, now, now, now); err != nil {
				return err
			}
			...
			return err
		} else if _, err = tx.ExecContext(ctx, `UPDATE provider_models SET display_name=?,context_length=?,max_output_tokens=?,available=1,last_seen_at=?,updated_at=? WHERE id=?`, model.DisplayName, nullableInt(model.ContextLength), nullableInt(model.MaxOutputTokens), now, now, modelID); err != nil {
			return err
		}
	}
	if len(seen) == 0 {
