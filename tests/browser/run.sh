...
#
# Env overrides:
#   TILLER_BROWSER_WORKERS       isolated router/mock pairs and test shards (default 2)
...
workers=${TILLER_BROWSER_WORKERS:-${PLAYWRIGHT_WORKERS:-2}}
...
while read -r i router_port mock_port; do
    docker run --rm --network host \
        ...
        tiller-router-browser-tests:dev npx playwright test --shard="$((i + 1))/$workers" &
    echo "$i $!" >> "$pids_file"
done < "$ports_file"
